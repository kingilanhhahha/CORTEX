...
import { Volume2, VolumeX, MessageCircle, X, FastForward } from 'lucide-react';
...
const CosmicNPC: React.FC<CosmicNPCProps> = ({
  ...
  // Pause background music when dialogue is open
  useEffect(() => {
    if (isDialogueOpen) {
      console.log('💬 CosmicNPC dialogue opened - pausing background music');
      setIsConversationActive(true);
    } else {
      console.log('💬 CosmicNPC dialogue closed - resuming background music');
      setIsConversationActive(false);
    }
  }, [isDialogueOpen, setIsConversationActive]);

  // Cleanup: ensure music resumes if component unmounts while dialogue is open
  useEffect(() => {
    return () => {
      setIsConversationActive(false);
    };
                      ...
                      {messages.length > 1 && (
                        ...
                        >
                          <Button
                            ...
                            className="text-xs font-orbitron bg-transparent border-primary/50 text-primary hover:bg-primary/10 hover:border-primary/80 hover:text-primary transition-all duration-200 relative overflow-hidden group ml-2"
                          >
