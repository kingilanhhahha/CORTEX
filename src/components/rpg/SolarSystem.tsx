  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-gradient-cosmos">
      ...
      <motion.div
        ...
        className="hidden lg:block absolute left-4 xl:left-6 top-[18%] z-30 w-[280px] xl:w-[380px] pointer-events-none"
      >
        <div
      ...
      <div className="absolute top-1/2 left-1/2" style={{ transform: 'translate(-50%, -50%)' }}>
        {/* Sun core */}
        <motion.div
          className="w-16 h-16 sm:w-20 sm:h-20 lg:w-24 lg:h-24 bg-gradient-to-br from-yellow-300 via-orange-500 to-red-600 rounded-full relative"
        animate={{ 
          boxShadow: [
      ...
      {planets.map((planet, index) => {
        ...
        const BASE_SIZE = 64; // px baseline for planet diameter at size=1 (smaller for mobile)
            ...
            <motion.div
              ...
              {lore && (
                <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
                  <div className="relative bg-black/75 backdrop-blur-sm text-white text-xs rounded-lg px-3 sm:px-4 py-2 border border-white/10 shadow-xl w-[280px] sm:w-[320px] whitespace-normal">
                    {/* Arrow */}
                    <div className="absolute left-1/2 top-full -translate-x-1/2 w-0 h-0 border-l-6 sm:border-l-8 border-r-6 sm:border-r-8 border-t-6 sm:border-t-8 border-l-transparent border-r-transparent border-t-black/75" />
                    <div className="font-semibold mb-1 truncate text-xs sm:text-sm">{lore.title}</div>
                    <div className="truncate text-xs">Cadet: {lore.tagline}</div>
                    <div className="text-white/80 truncate text-xs">Navigator: {lore.teacher}</div>
                  </div>
                </div>
              )}
            </div>
      ...
      {/* Control Buttons */}
      <div className="absolute top-4 left-4 sm:top-6 sm:left-6 z-50 flex flex-col sm:flex-row gap-2 sm:gap-3">
        {onBack && (
          <Button
            onClick={onBack}
            variant="outline"
            className="touch-button text-xs sm:text-sm px-3 sm:px-4 py-2"
          >
            <span className="hidden xs:inline">← Return to Station</span>
        ...
        <Button
          ...
          variant="outline"
          className="touch-button bg-black/60 backdrop-blur-sm border-white/20 text-white hover:bg-black/80 text-xs sm:text-sm px-3 sm:px-4 py-2"
        >
          {isOrbiting ? '⏸️ Stop' : '▶️ Start'}
        </Button>
      </div>
