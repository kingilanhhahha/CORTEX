...
const RPGIndex = () => {
  ...
  // Philippines Map intro state
  const [showPhilippinesIntro, setShowPhilippinesIntro] = useState(false);
  const [philippinesIntroStep, setPhilippinesIntroStep] = useState(1);

  const { user, logout, isLoading: authLoading } = useAuth();
  const navigate = useNavigate();
  const { cadetAvatar, currentProgress, getOverallProgress } = usePlayer();
  const [showCharacterSelect, setShowCharacterSelect] = useState(false);
  const { playAudio: playConversationAudio, stopAudio: stopConversationAudio } = useConversationAudio();
  ...
  const playAudioOnce = (src: string | null) => {
    if (!src) return;
    try {
      playConversationAudio(src, 0.95);
    } catch (error) {
      console.error('Failed to play conversation audio:', error);
      // Fallback: try to play with basic Audio API
      try {
  ...
  const handleSolarIntroSkipAll = () => {
    // Stop any playing conversation audio
  ...
  const startIntroCinematic = () => {
    ...
    setShowIntro(true);
    try {
      playConversationAudio(getCadetAudioConstellation(1), 0.9);
    } catch (error) {
      console.error('Failed to start intro cinematic audio:', error);
    }
  };
  ...
  const handleIntroNext = () => {
    if (introStep < 6) {
      ...
      try {
        playConversationAudio(audioSrc, 0.95);
      } catch (error) {
        console.error('Failed to play intro step audio:', error);
      }
      return;
    }
    endIntroToSolar();
  ...
  // Philippines Map introduction conversation
  const philippinesIntroConfig: { [k: number]: { speaker: 'Cadet' | 'Co‑Pilot'; side: 'left' | 'right'; text: string; audio: string; speed?: number } } = {
    1: { 
      speaker: 'Cadet', 
      side: 'left', 
      text: 'Co-Pilot, while we were exploring the Solar System, I couldn\'t stop thinking about Earth... it\'s just so... different from all the other planets.', 
      audio: '', // Will use cadet audio
      speed: 24 
    },
    2: { 
      speaker: 'Co‑Pilot', 
      side: 'right', 
      text: 'Ah, Earth. The blue jewel of the solar system. It holds a special place in our navigation charts. What caught your attention, Cadet?', 
      audio: '', // Will use co-pilot audio
      speed: 22 
    },
    3: { 
      speaker: 'Cadet', 
      side: 'left', 
      text: 'There\'s this archipelago I saw in the data — the Philippines. It\'s like a constellation of islands, each one unique. I keep thinking... what if we could practice our rational functions there?', 
      audio: '', 
      speed: 24 
    },
    4: { 
      speaker: 'Co‑Pilot', 
      side: 'right', 
      text: 'Brilliant observation, Cadet! The Philippines archipelago is perfect for our training. Each region represents different mathematical challenges — from Rice Terraces to Chocolate Hills, each landmark teaches us something new about rational functions.', 
      audio: '', 
      speed: 22 
    },
    5: { 
      speaker: 'Cadet', 
      side: 'left', 
      text: 'So we can actually explore it? Like, use real-world locations to understand math better?', 
      audio: '', 
      speed: 24 
    },
    6: { 
      speaker: 'Co‑Pilot', 
      side: 'right', 
      text: 'Exactly. Our navigation computers have created a virtual training simulation based on Earth\'s Philippines. Each lesson is tied to a real landmark — it helps you see how rational equations apply to the world around us. Ready to begin?', 
      audio: '', 
      speed: 22 
    },
  };

  const startPhilippinesIntro = () => {
    setPhilippinesIntroStep(1);
    setShowPhilippinesIntro(true);
    // Audio will be handled in the conversation
  };

  const handlePhilippinesIntroNext = () => {
    if (philippinesIntroStep < 6) {
      const next = philippinesIntroStep + 1;
      setPhilippinesIntroStep(next);
      // Note: Audio files would need to be added for this conversation
      return;
    }
    // End intro and navigate to Philippines map
    setShowPhilippinesIntro(false);
    setPhilippinesIntroStep(1);
  ...
  const handleQuestSelect = (questId: string) => {
    ...
    if (questId === 'basic-solving') {
      // Start Philippines introduction conversation
      startPhilippinesIntro();
      return;
    }
              ...
              <div className={`absolute ${solarScript[solarIntroStep - 1].side === 'left' ? 'left-6 md:left-12' : 'right-6 md:right-12'} bottom-6 md:bottom-12 flex flex-col ${solarScript[solarIntroStep - 1].side === 'left' ? 'items-start' : 'items-end'} gap-3 pointer-events-none`}>
                <div className="pointer-events-auto w-[88vw] max-w-[640px]">
                  <DialogueBox
                    ...
                    onSkipAll={handleSolarIntroSkipAll}
                    showNext
                    autoPlay={false}
                    speed={24}
                    variant="inline"
  ...
  if (showQuestMap) {
    ...
    return (
      <CosmicBackground variant="deep-space" intensity="high">
        ...
        {/* Philippines Map Intro Overlay (multi-step) */}
        <AnimatePresence>
          {showPhilippinesIntro && (
            <motion.div
              className="fixed inset-0 z-50"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            >
              {(() => {
                const phCfg = philippinesIntroConfig[philippinesIntroStep] || philippinesIntroConfig[1];
                const phIsLeft = phCfg.side === 'left';
                return (
                  <div className={`absolute ${phIsLeft ? 'left-6 md:left-12' : 'right-6 md:right-12'} bottom-6 md:bottom-12 flex flex-col ${phIsLeft ? 'items-start' : 'items-end'} gap-3 pointer-events-none`}>
                    <div className="pointer-events-auto w-[88vw] max-w-[640px]">
                      <DialogueBox
                        speaker={phCfg.speaker}
                        text={phCfg.text}
                        onNext={handlePhilippinesIntroNext}
                        showNext
                        autoPlay
                        speed={phCfg.speed ?? 24}
                        variant="inline"
                      />
                    </div>

                    <motion.img
                      src={phIsLeft ? selectedCadetImg : loloChar}
                      alt={phCfg.speaker}
                      className="w-[220px] h-[220px] md:w-[360px] md:h-[360px] object-contain select-none"
                      initial={{ x: phIsLeft ? -30 : 30, opacity: 0 }}
                      animate={{ x: 0, opacity: 1 }}
                      exit={{ x: phIsLeft ? -30 : 30, opacity: 0 }}
                      transition={{ duration: 0.4 }}
                      draggable={false}
                    />
                  </div>
  ...
  return (
    <CosmicBackground variant="starfield" intensity="medium">
      ...
      <RPGHeader
        currentPlanet="Mathematical Device"
        onNavigate={handleNavigation}
      />
      ...
      {/* Hero Section */}
      <motion.section
        className="responsive-container py-8 sm:py-12 lg:py-16 text-center"
      ...
      >
        ...
        >
          <h2 className="font-orbitron font-bold responsive-heading bg-gradient-primary bg-clip-text text-transparent">
            CORTEX ADVENTURE
          </h2>
          <motion.div
            className="absolute -inset-2 sm:-inset-4 bg-gradient-primary rounded-lg opacity-10 blur-xl"
            animate={{ scale: [1, 1.1, 1] }}
            transition={{ duration: 2, repeat: Infinity }}
          />
        </motion.div>

        <p className="responsive-text text-muted-foreground mb-6 sm:mb-8 max-w-3xl mx-auto px-4">
        ...
        <motion.div
          ...
          className="responsive-flex justify-center items-center mobile-gap"
        >
          <Button 
            onClick={() => setShowQuestMap(true)} 
            className="touch-button px-6 sm:px-8 py-3 sm:py-4 text-base sm:text-lg bg-gradient-quest shadow-quest w-full sm:w-auto" 
            size="lg"
          >
            <Map className="w-5 h-5 mr-2 sm:mr-2" />
            <span className="hidden xs:inline">Enter Galaxy Map</span>
            <span className="xs:hidden">Galaxy Map</span>
          </Button>
          <Button 
            variant="outline" 
            size="lg" 
            className="touch-button px-6 sm:px-8 py-3 sm:py-4 text-base sm:text-lg holographic-border w-full sm:w-auto" 
            onClick={() => setShowSolar(true)}
          >
            <Play className="w-5 h-5 mr-2 sm:mr-2" />
            <span className="hidden xs:inline">Enter Solar System</span>
            <span className="xs:hidden">Solar System</span>
          </Button>
          <Button 
            variant="outline" 
            size="lg" 
          ...
          >
            ...
            <span className="hidden xs:inline">Drawing Solver</span>
            <span className="xs:hidden">Drawing</span>
          </Button>
        </motion.div>
      </motion.section>
      ...
      >
        <div className="text-center mb-6 sm:mb-8">
          <h3 className="font-orbitron font-semibold responsive-heading text-card-foreground mb-2 sm:mb-4">🚀 Active Missions</h3>
        ...
        <div className="responsive-grid">
            ...
            <HolographicCard variant="quest" animated>
              <div className="responsive-padding">
                <div className="flex items-center gap-3 mb-4">
                  <div className={`w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br ${getCurrentLessonInfo().color} rounded-full`} />
                  <div>
                    <h4 className="font-orbitron font-bold text-base sm:text-lg">In Progress</h4>
                    <p className="text-xs sm:text-sm text-muted-foreground">{getCurrentLessonInfo().planet} Lesson</p>
                  </div>
                </div>
                <h5 className="font-semibold mb-2 text-sm sm:text-base">{getCurrentLessonInfo().title}</h5>
                <p className="text-xs sm:text-sm text-muted-foreground mb-4">
                  {getCurrentLessonInfo().description}
                </p>
                <div className="space-y-3">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                    <span className="text-xs sm:text-sm text-cosmic-green">
                      Overall Progress: {getOverallProgress()}%
                    </span>
                    <Button size="sm" className="bg-gradient-quest w-full sm:w-auto" onClick={handleContinue}>
                      Continue
                    </Button>
                  </div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 1.3 }}>
            <HolographicCard variant="portal" animated>
              <div className="responsive-padding">
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-10 h-10 sm:w-12 sm:h-12 portal-effect rounded-full p-0.5">
                    <div className="w-full h-full bg-card rounded-full flex items-center justify-center">
                      <span className="text-lg sm:text-2xl">🎨</span>
                    </div>
                  </div>
                  <div>
                    <h4 className="font-orbitron font-bold text-base sm:text-lg">Drawing Solver</h4>
                    <p className="text-xs sm:text-sm text-muted-foreground">AI-Powered OCR</p>
                  </div>
                </div>
                <h5 className="font-semibold mb-2 text-sm sm:text-base">Hand-Drawn Equations</h5>
                <p className="text-xs sm:text-sm text-muted-foreground mb-4">
